...
RUN /usr/bin/yes | unminimize; \
  ...
  apt-get -y install sudo locales less dialog apt-utils man-db bind9-dnsutils ripgrep fzf zoxide tree bc guile-3.0 zsh fish libarchive-tools zip unzip 7zip zstd sudo tmux bat parallel vim emacs-nox git iproute2 procps lsb-release libnss3-tools curl httpie jq sqlite3; \
  apt-get -y install docker.io docker-compose docker-buildx podman podman-compose; \
