...
RUN /usr/bin/yes | unminimize; \
  apt-get update; \
  apt-get -y install locales less dialog apt-utils man-db bc guile-3.0 zsh fish zip unzip sudo tmux bat parallel vim emacs-nox git iproute2 procps lsb-release libnss3-tools curl httpie jq sqlite3; \
