...
apt-get update; \
  ...
  apt-get -y install locales less dialog apt-utils man-db bc guile-3.0 zsh fish zip unzip zstd sudo tmux bat parallel vim emacs-nox git iproute2 procps lsb-release libnss3-tools curl httpie jq sqlite3 docker.io docker-compose; \
...
curl -L https://releases.hashicorp.com/terraform/1.5.5/terraform_1.5.5_linux_amd64.zip > /tmp/terraform.zip; \
